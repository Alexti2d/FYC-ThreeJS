...
let camera, scene, renderer, parentHolo, maLampe, monEcran, rendererCV;
...
function init() {
  ...
  camera.position.set(0.01713, 0.7, 0.9618);
  camera.lookAt(0, 1, 0);
  ...
  rendererCV = new CSS3DRenderer();
  rendererCV.setSize(window.innerWidth, window.innerHeight);
  const controls = new OrbitControls(camera, rendererCV.domElement);

  container.appendChild(renderer.domElement);
  container.appendChild(rendererCV.domElement);
  ...
  window.addEventListener("click", (event) => {
    ...
    const intersectionEcran = raycaster.intersectObject(monEcran);
    if (intersectionEcran.length > 0) {
      camera.position.set(
        -0.02189168485031384,
        0.2317157782473293,
        0.5531018786903674
      ...
      camera.rotation.set(
        -0.052991232297638365,
        0.06516297781261929,
        0.003453843033196438
      );
    }
  });
  CreationCV();
...
function CreationCV() {
  ...
  group.add(new CV(-0.278, 0.262, -0.060, 0.075));

  window.addEventListener("click", (event) => {
    const intersectionEcran = raycaster.intersectObject(group);
    if (intersectionEcran.length > 0) {
      camera.position.set(
        -0.02189168485031384,
        0.2317157782473293,
  ...
  group.add(new FondCV(-0.278, 0.262, -0.061, 0.077));
  scene.add(group);
...
function onWindowResize() {
  ...
  rendererCV.render(scene, camera);
}
